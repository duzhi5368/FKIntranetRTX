...
namespace IPMessagerNet
{
	static class Program
	{
		...
		static void Main(string[] cmd)
		{
			...
			//加载配置
			Env.ClientConfig = Core.ProfileManager.LoadConfig<ClientConfig>();
			if (Env.ClientConfig == null)
			{
				Env.ClientConfig = GetDefaultConfig();
				Env.ClientConfig.Save();
			}
			Env.ClientConfig.IPMClientConfig.VersionInfo = "飞鸽传书.Net " + Application.ProductVersion + "，BY FreeKnight";
