...
namespace FSLib.IPMessager
{
	...
	public static class IpmEvents
	{
		...
		static void SendOrPostCallback(bool sync, EventHandler handler, IPMClient client)
		{
			...
			{
				if (sync) client.ThreadContext.Send(_ => handler(client, System.EventArgs.Empty), null);
				else client.ThreadContext.Post(_ => handler(client, System.EventArgs.Empty), null);
			}
			else
			{
				handler(client, System.EventArgs.Empty);
			}
		}
		...
		static void SendOrPostCallback<T>(bool sync, EventHandler<T> handler, IPMClient client, T args) where T : System.EventArgs
		{
			if (handler == null) return;
		...
		internal static void OnUdpNetworkError(IPMClient client, System.EventArgs e)
		{
