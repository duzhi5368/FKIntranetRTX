...
namespace FSLib.IPMessager.Debug
{
	...
	{
		bool m_bIsAlreadyAttached = false;      // 是否已经挂接了一个客户端
		...
		{
            if(e.Data != null)
                System.Diagnostics.Debug.WriteLine("UDP数据包已经被过滤。远程主机：" + e.IPEndPoint.ToString() + "，数据包长度：" + e.Data.Length.ToString() + "，数据内容：" + BitConverter.ToString(e.Data).Replace("-", ""));
		}
		...
		{
            if (e.Data != null)
                System.Diagnostics.Debug.WriteLine("已收到UDP数据包，正在请求对来源主机信息进行验证。远程主机：" + e.IPEndPoint.ToString() + "，数据包长度：" + e.Data.Length.ToString());
		}
