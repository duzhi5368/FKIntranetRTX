...
namespace FSLib.IPMessager
{
	...
	{
		private bool m_bIsAlreadyInitialized = false;       // 是否已经被初始化过
		...
		public bool IsInitialized
		{
			get
			{
				return this.m_bIsAlreadyInitialized;
			}
		}
		...
		private void Initialize(Config cfg)
		{
			...
			this.m_bIsAlreadyInitialized = Commander.IsInitialized;
		}
		...
		protected virtual void Dispose(bool disposing)
		{
			if (!this._disposedValue)
			{
				if (disposing)
				{
					if (this.m_bIsAlreadyInitialized)
					{
						OffLine();
						Commander.Dispose();
						this.m_bIsAlreadyInitialized = false;
					}
				}
			...
			return new Config
				{
					...
					VersionInfo = String.Format("飞鸽传书.Net {0}，BY FreeKnight", System.Diagnostics.FileVersionInfo.GetVersionInfo(System.Reflection.Assembly.GetExecutingAssembly().Location).FileVersion),
					AbsenceSuffix = " [离开]",
